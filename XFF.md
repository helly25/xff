...
- `--case=sensitive|insensitive|smart, -i, -s[+|-]` - letter case for matchers: -i insensitive, -s/-s+ smart, -s- sensitive (rg -> smart) _(global, xff)_
...
- `--regextype=RE2|EXACT|FNMATCH|GLOB|SHGLOB|PCRE2` - match engine: RE2, EXACT, FNMATCH, GLOB, SHGLOB (GLOB + {a,b}), or PCRE2 (a build extra) _(global, xff)_
...
- `--skip-vcs[=git,hg,svn,jj,bzr,darcs,cvs|all|none]` - prune VCS metadata dirs (.git, .hg, ...); bare/=all = every known VCS, =LIST a subset _(global, xff)_
...
### Output
- `--format=plain|nul|jsonl|csv|tsv|aligned|markdown|tree` - output format: plain, nul, jsonl, csv, tsv, aligned, markdown (md), tree; default plain _(global, xff)_

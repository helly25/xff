...
- Macros: `UPPER_CASE`, prefixed `XFF_` (`XFF_...`). (A macro you call from the mbo
  dependency keeps its own `MBO_` prefix, e.g. `MBO_RETURN_IF_ERROR`; the rule is about
  macros xff itself defines.)
...
- Macros: avoid them. An **exported** macro (defined in a header for other code to use) is
  prefixed `XFF_` (private impl `XFF_PRIVATE_...`). A macro **local to one translation unit**
...
- Library flags are prefixed by their path/namespace (e.g. a flag in `xff/<pkg>` is
  `--xff_<pkg>_...`). The application entry-point binary (`xff`, also invoked as `find`) uses
  bare flag names (`--config`, `--summary`, `--define`).

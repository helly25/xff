...
TEST_F(FuseLoaderTest, TheRequiredSetCoversTheServersCallSurface) {
  // The server (slice 3) needs the session lifecycle, the mount pair, and the five reply calls; a
  // shrink of this list would let "available" lie. SizeIs guards accidental deletion; the names
  // themselves are the SOT in fuse_loader.cc.
  EXPECT_THAT(RequiredSymbols(), SizeIs(12));
}

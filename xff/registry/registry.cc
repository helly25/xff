...
constexpr std::array kDescriptors = std::to_array<Descriptor>({
    ...
    // [/ENCODING]` picks the algorithm / encoding, like -hash. `! -hasheq` selects files that drift.
    {
        ...
        .details = "Computes the file's digest and is true when it equals EXPECTED - a {field} template evaluated per "
                   "entry, so it can name a sidecar value like `{def.SUMS}` or a capture. `-hasheq=ALGO[/ENCODING]` "
                   "picks the algorithm (sha256 default; also sha1/sha512/...) and encoding (hex default, or base64); "
                   "the same grammar as -hash / {hash}. It is a strict equality test (hex folds case). `! -hasheq` "
                   "selects files whose digest differs (drift / corruption). Reads the whole file, so it is expensive.",
        .kind = Kind::kTest,
        .arity = 1,
        .binding = Binding::kHash,
        .style = Style::kXff,

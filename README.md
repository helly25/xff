...
- **Virtual Archive Filesystems:** `--archive` walks archives and compressed files as directory trees, including nested containers and Electron ASAR application bundles. The normal name, metadata, content, hash, and reduction vocabulary works on members; the removable ASAR extra understands packed data, unpacked sidecars, links, and integrity records without presenting incidental metadata as files. Opt-in controls can mount or extract members for commands and safely rewrite formats that support deletion.

...
- **Native Comparison & Deduplication:** `-cmp` tests byte equality, `-hasheq` verifies an expected digest, `--summary=verification` tallies verified and failed checks in that same walk, `-diff` produces unified, context, normal, or side-by-side diffs, and `--summary=hash` groups duplicate content without launching one process per file.
...
| **Single-pass hash verification tally** |   -    |  -   |  -   |   -   |   -    |  -   |   -    |     △      |       -       | **✓ `--summary=verification`**           |

...
#
# The result is consumed as the @msan_libcxx REPOSITORY (see MODULE.bazel + bazelmod/
# msan_libcxx.BUILD), whose cc_library gives every wrapped target the instrumented headers
# (execroot-relative -isystem, ordinary include chain) and archives. Earlier attempts named
# this directory in copts/linkopts and failed three ways: %workspace% is not expanded in a
# copt value, an absolute -isystem is "outside of the execution root", and -cxx-isystem
# breaks libc++'s C-header shims, which need #include_next to reach glibc.
#

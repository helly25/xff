...
class CheckTest(unittest.TestCase):
    ...
    def test_an_allowlisted_library_is_permitted(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            build = self._write(root, "xff/cli/BUILD.bazel", 'cc_library(\n    name = "main_cc",\n)\n')
            self.assertEqual(cclt.check([build], root), [])

    def test_the_allowlist_entry_for_main_cc_records_a_reason(self):
        # An allowlist without reasons decays into a list of things nobody dares remove.
        self.assertIn("xff/cli:main_cc", cclt._ALLOWLIST)
        self.assertIn("bashtest", cclt._ALLOWLIST["xff/cli:main_cc"])
